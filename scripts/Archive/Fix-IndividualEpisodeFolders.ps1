# Fix-IndividualEpisodeFolders.ps1
# Consolidates shows that have individual folders per episode
# Creates proper season folders and moves all episodes into them
...
Write-Host "Individual Episode Folders Cleanup" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
foreach ($showInfo in $showsToFix) {
    ...
    foreach ($episodeFolder in $episodeFolders) {
        ...
        # Find video files in this episode folder
        ...
        if (-not $DryRun) {
            Start-Sleep -Milliseconds 100  # Brief pause to ensure filesystem catches up
            $remainingFiles = Get-ChildItem -Path $episodeFolder.FullName -Recurse -File
            ...
            Write-Host "      [DRY RUN] Would remove: $($episodeFolder.Name)" -ForegroundColor Gray
        }
    }
